...
_npm_copy() {
    ...
    cp -r "${FARM_DIR}/node_modules/*" "node_modules/"
}
...
_rebuild_npm() {
    ...
    hashcheck "${FARM_DIR}/node_modules" package.json "_npm_copy" "NPM copy shortcut"
    npm install
}
...
if [[ ! -f "$OPTIONS_DIR/norefresh" && -d /app/.git ]]; then
    ...
    hashcheck bower_components bower.json "bower install --allow-root" "bower"
